	...
	<body>
		...
		<div class="row">
			<div class="12u">
			 ...
			 | 		<a href = "/help">Help</a>  <?php if ($this->Session->read('Auth.User')){echo '| Logged in as '.AuthComponent::user('username') .' |'.$this->Html->link('Logout','/users/logout', array('class' => 'button','controller' => 'users', 'action' => 'logout')). '|' . $this->Html->link('Settings','/users', array('class' => 'button','controller' => 'users', 'action' => 'index'));} ?>
				</div>
			</div>
		</div>
	</body>
